                                                                                                            ...
                                                                                                            pkgs.stdenv.mkDerivation
                                                                                                                {
                                                                                                                    installPhase =
                                                                                                                        ...
                                                                                                                        ''
                                                                                                                            ...
                                                                                                                            ln --symbolic ${ pkgs.writeShellScript "application" ( builtins.concatStringsSep "\n" ( builtins.map ( secretFile : ''mkdir --parents $( dirname secretFile ) && age --decrypt --identity ${ config.personal.agenix } --output ${ secretFile } $OUT/src/${ secretFile } && chmod 0400 ${ secretFile }'' ) secretFiles ) ) } $out/scripts/application
                                                                                                                            makeWrapper $out/scripts/application $out/bin/application --set PATH ${ pkgs.lib.makeBinPath [ pkgs.age pkgs.coreutils ] } --set OUT $out
                                                                                                                        ''  ;
                                                                                                                    name = "derivation" ;
