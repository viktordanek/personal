                                                                                                    ...
                                                                                                    let
                                                                                                        post-commit =
                                                                                                            pkgs.writeShellScript
                                                                                                                "post-commit"
                                                                                                                ''
                                                                                                                    ${ pkgs.jq }/bin/jq --null-input --arg branch "$( ${ pkgs.git }/bin/git rev-parse --abbrev-ref HEAD )" --arg COMMIT_HASH "$( ${ pkgs.git }/bin/git rev-parse --abbrev-ref HEAD )" --arg TREE "${ _environment-variable "TEMPORARY" }/${ value.user-name }/tree" --arg ORIGIN "${ value.origin }" '{ branch : $BRANCH , commit_hash : $COMMIT_HASH , tree : $TREE , origin : $ORIGIN }" | ${ pkgs.jq }/bin/jq --raw-input  | ${ pkgs.redis }/bin/redis-cli PUBLISH git-commit-received
                                                                                                                '' ;
                                                                                                        in
                                                                                                            pkgs.writeShellScript
                                                                                                                "script"
