                                                                        ...
                                                                        pkgs.writeShellApplication
                                                                            {
                                                                                ...
                                                                                text =
                                                                                    let
                                                                                        ...
                                                                                        yaml =
                                                                                            code :
                                                                                                ''jq --null-input --arg CODE "${ builtins.toString code }" --arg EXPECTED "${ builtins.concatStringsSep "\n" resource.outputs }" --arg INIT_SCRIPT "${ resource.init-script }" --arg OBSERVED "$( find "$STASH/mount" -mindepth 1 -maxdepth 1 -exec basename {} \; | sort )" --arg OUTPUT "${ builtins.concatStringsSep "," resource.outputs }" --arg release-script "${ resource.release-script }" --arg STANDARD_ERROR "$( cat "$STASH/standard-error" )" --arg STANDARD_OUTPUT "$( cat "$STASH/standard-output" )" --arg STATUS "$?" '{ "code" : $CODE , "expected" : $EXPECTED , "observed" : $OBSERVED , "init-script" : $INIT_SCRIPT , "release-script" : $RELEASE_SCRIPT ,"standard-error" : $STANDARD_ERROR , "standard-output" : $STANDARD_OUTPUT , "status" : $STATUS }' | yq --yaml-output "." > "$STASH/${ if code == 0 then "success" else "failure" }.yaml"'' ;
                                                                                        in
                                                                                            ''
